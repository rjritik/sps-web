...
const DashboardQuarryDetails = () => {
  ...
  // Apply search filter on refNumber
  const filteredBlocks = useMemo(() => {
    if (!searchTerm) return blockByQuarry || [];
    return (blockByQuarry || []).filter((block) =>
      block.refNumber?.toLowerCase().includes(searchTerm.toLowerCase())
    );
  }, [blockByQuarry, searchTerm]);
    ...
    <DashboardLayout>
      ...
      <div className="flex flex-wrap justify-between items-center gap-4 mb-6">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="text-sm text-gray-1 font-medium">
            Total {filteredBlocks?.length || 0} {quarryName}
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-6">
        {filteredBlocks.length > 0 ? (
          ...
          <div className="text-gray-500 ml-2">No Blocks found.</div>
        )}
      </div>
